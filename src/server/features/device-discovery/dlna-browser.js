...
const buildRequestXml = function (id, options) {
  ...
  return xmlbuilder
    .create('s:Envelope', { version: '1.0', encoding: 'utf-8' })
    ...
    .ele('ObjectID', id)
    .up()
    .ele('BrowseFlag', options.browseFlag)
    .up()
    .ele('Filter', options.filter)
    .up()
    .ele('StartingIndex', options.startIndex)
    .up()
    .ele('RequestedCount', options.requestCount)
    .up()
    .ele('SortCriteria', options.sort)
    .doc()
    ...
    response.on('end', function () {
      ...
      xmltojs.parseString(data, function (err, result) {
        ...
        if (
          ...
          xmltojs.parseString(
            ...
            function (err, listResult) {
              ...
              if (listResult['DIDL-Lite']) {
                ...
                callback(
                  new Error(
                    'Did not get expected listResult from server:' + result
                  )
                );
                return;
              }
          ...
          callback(
            new Error('Did not get expected response from server:' + result)
          );
          return;
        }
      });
