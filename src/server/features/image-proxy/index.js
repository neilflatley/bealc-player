...
let whitelist = process.env.WHITELIST || []; // [/\.gov$/, /google\.com$/]
...
const imageProxy = app => {
  app.get(
    '/image/:url/:width?/:height?.:extension?',
      ...
      var width = req.params.width,
        ...
        retrieve = function (remote) {
          ...
          if (options.protocol !== 'http:' && options.protocol !== 'https:') {
            return res
              .status(404)
              .send('Expected URI scheme to be HTTP or HTTPS');
          }
          if (!options.hostname) {
            return res.status(404).send('Expected URI host to be non-empty');
          }
          options.headers = {
            'User-Agent': 'image-proxy/0.0.7',
            Accept: '*/*',
          };
              ...
              .get(options, function (response) {
                ...
                if (response.statusCode !== 200) {
                  return res
                    .status(404)
                    .send(
                      'Expected response code 200, got ' + response.statusCode
                    );
                }
        ...
        if (parts.hostname) {
          ...
          for (_i = 0, _len = whitelist.length; _i < _len; _i++) {
            if (typeof whitelist[_i] === 'string') {
              ...
              whitelist[_i] = new RegExp(
                whitelist[_i].replace('.', '\\.') + '$'
              );
            }
